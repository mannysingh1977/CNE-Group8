...
const main = async () => {
  // Delete data in reverse order of dependency to avoid conflicts.
  ...
  const John = await prisma.user.create({
    data: {
      ...
      newsLetter: true,
      role: "Owner",
      address: { connect: { id: addressJhon.id } },
    },
  });
  ...
  const Jane = await prisma.user.create({
    data: {
      ...
      newsLetter: true,
      role: "User",
      address: { connect: { id: addressJane.id } },
    },
  });
  ...
  const Mike = await prisma.user.create({
    data: {
      ...
      newsLetter: false,
      role: "User",
      address: { connect: { id: addressMike.id } },
    },
  });
  ...
  const Emily = await prisma.user.create({
    data: {
      ...
      newsLetter: false,
      role: "Admin",
      address: { connect: { id: addressEmily.id } },
    },
  });

  // Create additional users along with their addresses
  const users = [
    {
      name: "Alice Johnson",
  ...
  // Create Chris user using pre-created address
  ...
  const Chris = await prisma.user.create({
    data: {
      ...
      newsLetter: true,
      role: "User",
      address: { connect: { id: addressChris.id } },
    },
  });
